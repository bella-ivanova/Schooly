...
- Database schema: 15 migrations defined (PostgreSQL), most recent is `ReplaceClassIdWithClassStudentJoinTable` (adds `ClassStudent`/`ClassJoinCode`, backfills existing single-class assignments, drops `ApplicationUser.ClassId`); run `dotnet ef database update` before starting the server
...
- School admin HTTP endpoints: `GET /api/admin/classes`, `POST /api/admin/classes`, `PUT /api/admin/classes/{classId}/homeroom`, `POST /api/admin/classes/{classId}/students`, `DELETE /api/admin/classes/{classId}/students/{userId}` (removes one specific class membership — a student may belong to several), `POST /api/admin/classes/{classId}/teachers`, `POST /api/admin/teachers/{teacherId}/subjects/{subjectId}`, `DELETE /api/admin/teachers/{teacherId}/subjects/{subjectId}`, `GET /api/admin/subjects`, `GET /api/admin/users` (each user's `classNames` is now a list, not a single nullable name) — requires SchoolAdmin JWT
...
- Per-role dashboard home screens: one route per role (`/app/student`, `/app/teacher`, `/app/school-admin`, `/app/global-admin`), each an `AppShell`-wrapped summary view backed by a dedicated API wrapper (`frontend/src/api/{student,teacher,schoolAdmin,globalAdmin}.ts`) — student sees recent activity + weak spots + a "My Classes" card that lists joined classes and lets them self-join another one by code (`GET/POST /api/student/classes`), teacher sees a class picker with struggles/activity plus a generate/regenerate control for the selected class's join code (`GET/POST /api/teacher/classes/{id}/join-code[/regenerate]`), school admin sees a class table, a working "Create class" form (name + homeroom-teacher picker, `POST /api/admin/classes`), and a working teacher-registration-code generate/regenerate control (`GET/POST /api/admin/teacher-code[/regenerate]`), global admin sees a school list (`GET /api/global-admin/schools`). Sidebar nav items beyond the active "home" entry (new chat, past chats, subject/user management, curriculum files) are present but `disabled: true` placeholders — most other write endpoints (homeroom reassignment, manual student/teacher assignment, curriculum upload, user role change, school creation, etc.) still have no frontend API wrapper.
...
### Not Started
- Frontend: chat UI, the `<STEREO>` 3D viewer, RAG source-citation UI, and most remaining dashboard *action* UIs (homeroom reassignment, manual student/teacher assignment, subject/user/curriculum management, and their backing write-endpoint API wrappers) — class creation and the class-join-code flow are the exceptions, both wired end-to-end
