...
Controllers/        HTTP endpoints only — no business logic, no DB access
  ...
  SchoolAdminController.cs      /api/admin — school admin class, subject, and assignment management (SchoolAdmin role only), plus GET/POST /api/admin/teacher-code[/regenerate] for the school's teacher registration code
...
frontend/           Vue 3 + Vite + TypeScript SPA — dev server on :5173 (npm run dev), not a separate repo
  src/api/            Typed API client — no UI framework imports here
    ...
    schoolAdmin.ts          getSchool/getClasses/getUsers/createClass/getTeacherCode/regenerateTeacherCode — backs SchoolAdminHome.vue; no wrapper yet for the remaining write endpoints (homeroom reassignment, assign/remove student, assign teacher, subject-teacher link) or GET /api/admin/subjects
  ...
  src/components/dashboard/
    ...
    schooladmin/   SchoolAdminHome.vue (class table + a working "Create class" form — name + homeroom-teacher `SelectField` sourced from `GET /api/admin/users` — calling `schoolAdmin.ts`'s `createClass`, plus the teacher registration code display with a working "Regenerate" button; per-row "Set homeroom · Add/remove student" actions are still static text, not wired)
    globaladmin/   GlobalAdminHome.vue (school list from `GET /api/global-admin/schools`) + SchoolCard.vue
    All four are read-only summary views wired to real endpoints — not stubs — but only Student/Teacher/SchoolAdmin have any write action wired (join a class, generate a class code, create a class, respectively); most other create/edit/delete UI is still future work, not a placeholder oversight.
