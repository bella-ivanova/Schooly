...
Models/             EF Core entity definitions and enums — no business logic
  ...
  RefreshToken.cs, PasswordResetCode.cs, SchoolTeacherCode.cs, RateLimitEntry.cs back the auth/security flows described under Security Requirements below. SchoolTeacherCode.cs (SchoolId FK, Code, IsActive, RevokedAt) is the per-school teacher registration code — persistent/reusable rather than single-use, unlike PasswordResetCode.cs.
