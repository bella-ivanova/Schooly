...
Controllers/        HTTP endpoints only — no business logic, no DB access
  ...
  TeacherDashboardController.cs /api/teacher — teacher class list, struggle topics, student activity, plus GET/POST /api/teacher/classes/{classId}/join-code[/regenerate] (Teacher role only) for that class's student-facing `ClassJoinCode`
  ...
  StudentController.cs          /api/student — student's own practice questions, weak spots, chat history, mock exam generation, and GET/POST /api/student/classes (list joined classes / self-join a class via a `ClassJoinCode`) (Student role only)
...
frontend/           Vue 3 + Vite + TypeScript SPA — dev server on :5173 (npm run dev), not a separate repo
  src/api/            Typed API client — no UI framework imports here
    ...
    student.ts             getWeakSpots/getHistory/getClasses/joinClass — backs StudentHome.vue, including the class-join-code self-service flow; no wrapper yet for POST /api/student/practice-questions or /api/student/exam
    teacher.ts              getClasses/getStruggles/getActivity/getClassJoinCode/regenerateClassJoinCode — backs TeacherHome.vue; covers all of TeacherDashboardController
  ...
  src/components/shell/
    ...
    navItem.ts       `NavItem` type ({ label, active?, disabled?, primary? }); every current nav entry across all four roles is `disabled: true` except the active "home" item — they're placeholders for not-yet-built feature routes (chat, class/subject/user management, curriculum files)
  src/components/dashboard/
    ...
    teacher/       TeacherHome.vue (class picker; loads `GET /api/teacher/classes|activity` on mount, `GET /api/teacher/classes/{id}/struggles|join-code` on selection, with a Generate/Regenerate control for that class's `ClassJoinCode`) + ClassCard.vue + StruggleTopicsCard.vue + MostActiveStudentsCard.vue
    ...
    All four are read-only summary views wired to real endpoints — not stubs — but only Student/Teacher/SchoolAdmin have any write action wired (join a class; generate a class code; create/edit a class, its subjects, roster, and teacher assignments, respectively); most other create/edit/delete UI (homeroom reassignment, teacher-subject qualification, user/role management) is still future work, not a placeholder oversight.
  ...
  src/views/  LoginView.vue, RegisterView.vue, ForgotPasswordView.vue (auth screens) + StudentHomeView.vue, TeacherHomeView.vue, SchoolAdminHomeView.vue, GlobalAdminHomeView.vue (one per role, each a thin wrapper: `AppShell` + that role's `NavItem[]` + the matching `components/dashboard/*/​*Home.vue` component; there is no longer a single shared `HomeView.vue` — routing to the right one is `router/index.ts`'s job, not a v-if inside one view)
```
