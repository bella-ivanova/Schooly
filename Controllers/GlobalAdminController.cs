...
public class GlobalAdminController : ControllerBase
{
    ...
    public async Task<IActionResult> CreateClass([FromBody] GlobalCreateClassRequest body)
    {
        ...
        var (success, error) = await _admin.AddClassAsync(body.SchoolId, body.Name, body.SubjectId, body.HomeroomTeacherId, body.Grade);
        if (!success) return BadRequest(new { error });
...
public record GlobalCreateClassRequest(int SchoolId, string Name, int SubjectId, string? HomeroomTeacherId, int? Grade);
